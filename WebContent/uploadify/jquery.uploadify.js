...
(function($) {
			...
			return this
					.each(function() {
						...
						var settings = $.extend({
							...
							fileObjName : 'Filedata', // The name of 这个文件
							...
							itemTemplate : false, // The template for 这个文件
													// item in the queue
						...
						if (flashInstalled) {
							...
							swfuploadify.queueData = {
								...
								uploadQueue : [], // 这个文件s currently to be
													// uploaded
								errorMsg : '添加任务队列错误'
							};
	...
	// These functions handle all the events that occur with 这个文件 uploader
	var handlers = {

		// Triggered when 这个文件 dialog is opened
		...
		onSelect : function(file) {
			...
			for ( var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('这个文件 named "'
							+ file.name
							+ '" 文件好像是只读的  不能上传?');
					if (!replaceQueueItem) {
			...
			// Truncate 这个文件name if it's too long
			var fileName = file.name;
			if (fileName.length > 25) {
				fileName = fileName.substr(0, 25) + '...';
			}

			// Create 这个文件 data object
			itemData = {
				'fileID' : file.id,
		...
		// Triggered when all 这个文件s in the queue have been processed
		...
		onUploadComplete : function(file) {
			...
			// Check if all 这个文件s have completed uploading
